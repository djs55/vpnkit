...
let hvsock_create () =
  let rec loop () =
    ...
    | exception e ->
        ...
        Mirage_sleep.ns (Duration.of_sec 1) >>= fun () -> loop ()
  in
  loop ()
  ...
  let rec aux () =
    ...
    Lwt.catch
      ...
      (fun e ->
        ...
        >>= fun () -> Mirage_sleep.ns (Duration.of_sec 1))
    >>= fun () -> aux ()
  in
  aux ()
...
let main_t configuration socket_url port_control_urls introspection_urls
  ...
  Log.info (fun f -> f "Version is %s" (Lazy.force Version.version));
  ...
  match Uri.scheme uri with
  | Some ("hyperv-connect" | "hyperv-listen") ->
      let module Slirp_stack = Slirp.Make (Vmnet.Make (HV)) (Dns_policy) (Vnet)
      in
      let sockaddr =
...
let term, info =
  ...
  in
  ( Term.(
      const main $ socket $ port_control_urls $ introspection_urls
    ...
    Cmd.info
      ...
      ~version:(Lazy.force Version.version)
      ~doc ~man )
