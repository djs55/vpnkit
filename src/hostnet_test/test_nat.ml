...
let run ?(timeout = Duration.of_sec 60) t =
  let timeout = Mirage_sleep.ns timeout >>= fun () -> Lwt.fail_with "timeout" in
...
module UdpServer = struct
  ...
  let make stack port =
    ...
    Client.UDP.listen (Client.udp stack) ~port
  ...
  let wait_for_data ~highest t =
    if t.highest < highest then
      Lwt.pick [ Lwt_condition.wait t.c; Mirage_sleep.ns (Duration.of_sec 1) ]
        ...
        with_stack ~pcap:"test_udp.pcap" (fun _ stack ->
            ...
            let rec loop remaining =
              ...
              Client.UDP.write ~src_port:virtual_port
                ~dst:(V4 Ipaddr.V4.localhost) ~dst_port:local_port udpv4 buffer
              >>= function
              | Error e -> err_udp e
              | Ok () -> (
    ...
    EchoServer.with_server (fun { EchoServer.local_port; _ } ->
        with_stack ~pcap:"test_udp_expiry.pcap" (fun slirp_server stack ->
            ...
            let rec spam from_port remaining =
              ...
              | 0 -> Lwt.return_unit
              | n -> (
                  Client.UDP.write ~src_port:(from_port + n)
            ...
            let rec loop remaining =
              ...
              Log.debug (fun f ->
                  f "Sending %d -> %d value %d" virtual_port1 local_port
                    (Cstruct.get_uint8 buffer 0));
              Client.UDP.write ~src_port:virtual_port1
                ~dst:(V4 Ipaddr.V4.localhost) ~dst_port:local_port udpv4 buffer
              >>= function
              | Error e -> err_udp e
              | Ok () -> (
            ...
            let rec loop remaining =
              ...
              Log.debug (fun f ->
                  f "Sending %d -> %d value %d" virtual_port2 local_port
                    (Cstruct.get_uint8 buffer 0));
              Client.UDP.write ~src_port:virtual_port2
                ~dst:(V4 Ipaddr.V4.localhost) ~dst_port:local_port udpv4 buffer
              >>= function
              | Error e -> err_udp e
              | Ok () ->
            ...
            let rec loop remaining =
              ...
              Log.debug (fun f ->
                  f "Sending %d -> %d value %d" virtual_port1 dst_port
                    (Cstruct.get_uint8 buffer 0));
              Client.UDP.write ~src_port:virtual_port1
                ~dst:(V4 Ipaddr.V4.localhost) ~dst_port udpv4 buffer
              >>= function
              | Error e -> err_udp e
              | Ok () -> (
                  UdpServer.wait_for_data ~highest:1 server1 >>= function
                ...
                let rec loop remaining =
                  ...
                  Log.debug (fun f ->
                      f "Sending %d -> %d value %d" virtual_port local_port
                        (Cstruct.get_uint8 buffer 0));
                  Client.UDP.write ~src_port:virtual_port
                    ~dst:(V4 Ipaddr.V4.localhost) ~dst_port:local_port udpv4
                    buffer
                  >>= function
                  | Error e -> err_udp e
                  | Ok () -> (
                ...
                let rec loop remaining =
                  ...
                  Log.debug (fun f ->
                      f "Sending %d -> %d value %d" virtual_port local_port1
                        (Cstruct.get_uint8 buffer 0));
                  Client.UDP.write ~src_port:virtual_port
                    ~dst:(V4 Ipaddr.V4.localhost) ~dst_port:local_port1 udpv4
                    buffer
                  >>= function
                  | Error e -> err_udp e
                  | Ok () -> (
                      Log.debug (fun f ->
                          f "Sending %d -> %d value %d" virtual_port local_port2
                            (Cstruct.get_uint8 buffer 0));
                      Client.UDP.write ~src_port:virtual_port
                        ~dst:(V4 Ipaddr.V4.localhost) ~dst_port:local_port2
                        udpv4 buffer
                      >>= function
                      | Error e -> err_udp e
                      | Ok () -> (
