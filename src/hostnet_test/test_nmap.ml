  ...
  let t _ stack =
    ...
    let connect_disconnect ip port =
      ...
      | Ok flow ->
          ...
          Client.TCP.close flow >>= fun () -> Lwt.return_unit
    in
    ...
    let rec loop seq =
      if Queue.length Client.Icmpv41.packets > 0 then Lwt.return_unit
        ...
        Client.Icmpv41.write stack.Client.icmpv4
          ~dst:(Ipaddr.to_v4 localhost_ip |> Option.get)
          ping
        >>= function
        | Error e -> failf "Icmpv41.write failed: %a" Client.Icmpv41.pp_error e
        | Ok () ->
            Mirage_sleep.ns (Duration.of_sec 1) >>= fun () -> loop (seq + 1)
    in
