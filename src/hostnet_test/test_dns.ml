...
let test_noerror_empty_response server config () =
  let name = "ipv4.tlund.se" in
  (* FIXME: Use a better address? *)
  let t _ stack =
    set_dns_policy config;
    let resolver = DNS.create stack.Client.t in
    (* Query for an AAAA record on a domain that might only have A records *)
    DNS.gethostbyname ~server ~q_type:Dns.Packet.Q_AAAA resolver name
    >>= function
    | [] ->
    ...
    | _ ->
        Log.err (fun f ->
            f "Expected no AAAA records for %s, but got some" name);
...
let test_nxdomain_response server config () =
  let name = "nonexistent.example.com" in
...
let udp_rpc client src_port dst dst_port buffer =
  ...
  let send_request () =
    Client.UDP.write ~src_port ~dst ~dst_port udpv4 buffer >>= function
    | Error e -> err_udp e
    | Ok () -> Lwt.return_unit
  in

  let response = ref None in
  Client.UDP.listen (Client.udp client.Client.t) ~port:src_port
  ...
  let rec loop () =
    ...
    match !response with
    | Some x -> Lwt.return x
    | None -> Mirage_sleep.ns (Duration.of_sec 1) >>= fun () -> loop ()
  in
  loop ()
