...
module PortsServer = struct
  ...
  let with_server f =
    ...
    f port >>= fun () -> Host.Sockets.Stream.Tcp.stop server
end
...
let udp_echo t len =
  ...
  let sender () =
    let rec loop () =
      ...
      | Ok () -> Mirage_sleep.ns (Duration.of_sec 1) >>= fun () -> loop ()
    in
    loop ()
  in
  let receiver () =
  ...
  let t _ stack =
    ...
    Lwt.finalize
      (fun () ->
        ...
        Client.TCP.create_connection
          (Client.tcp stack.Client.t)
      ...
      (fun () -> Host.Sockets.Stream.Tcp.stop server)
  in
