...
let send_http_request stack (ip, port) request =
  ...
  Client.TCP.create_connection (Client.tcp stack) (ip, port) >>= function
  | Ok flow ->
        ...
        (fun server ->
          ...
          | Ok () ->
              send_http_request stack.t
                (Ipaddr.of_string_exn "127.0.0.1", port)
                request
              >>= fun () ->
              Lwt.pick
                [
         ...
         with_server
           (fun flow ->
             ...
             | `Ok req -> (
                 ...
                 Alcotest.check
                   Alcotest.(option string)
                   "host"
                   (Some (Ipaddr.to_string test_dst_ip))
                   (Uri.host uri);
                 Alcotest.check
                   Alcotest.(option int)
           ...
           (fun server ->
             ...
             | Ok () -> (
                 ...
                 | Error _ ->
                     Log.err (fun f ->
                         f "TCPV4.create_connection %a:443 failed" Ipaddr.pp
                           test_dst_ip);
                 ...
                 | Error _ ->
                     Log.err (fun f ->
                         f "TCPV4.create_connection %s:%d failed"
                           (Ipaddr.to_string primary_dns_ip)
                           3129);
                     failwith "TCPV4.create_connection"
                 | Ok flow -> (
                     let oc = Outgoing.C.create flow in
                     let request =
                       ...
                       in
                       let resource =
                         Fmt.str "localhost:%d" server.Server.port
                       in
                       let headers =
                         Cohttp.Header.replace connect.Cohttp.Request.headers
                           "host" resource
                       in
                     ...
                     | `Eof ->
                         failwith
                           "test_http_connect_forward: EOF on HTTP CONNECT"
                     | `Invalid x ->
                         failwith
                           ("test_http_connect_forward: Invalid HTTP response: "
                          ^ x)
                     | `Ok res -> (
                         if res.Cohttp.Response.status <> `OK then
                           failwith
                             "test_http_connect_forward: HTTP CONNECT failed";
                         Outgoing.C.read_some ~len:5 oc >>= function
                         | Error e ->
                             Fmt.kstr failwith "%a" Outgoing.C.pp_error e
                         | Ok `Eof -> failwith "EOF"
                         | Ok (`Data buf) ->
                             let txt = Cstruct.to_string buf in
                             Alcotest.check Alcotest.string "message" "hello"
                               txt;
                             Lwt.return_unit))))))
    ...
    (Slirp_stack.with_stack ~pcap:"test_http_proxy_connect.pcap" (fun _ stack ->
         with_server
           (fun flow ->
             let ic = Incoming.C.create flow in
             (* read something *)
             Incoming.C.read_some ~len:5 ic >>= function
             | Ok `Eof -> failwith "http_proxy_connect: read_some returned Eof"
             | Error _ ->
                 failwith "http_proxy_connect: read_some returned Error"
             | Ok (`Data buf) -> (
                 let txt = Cstruct.to_string buf in
                 Alcotest.check Alcotest.string "message" "hello" txt;
                 ...
                 Incoming.C.flush ic >>= function
                 | Error _ ->
                     failwith "http_proxy_connect: flush returned error"
                 | Ok () ->
                 ...
                 {|
            {
             ...
             | Ok () -> (
                 ...
                 | Ok flow -> (
                     ...
                     | `Invalid x ->
                         failwith
                           ("test_proxy_connect: Invalid HTTP response: " ^ x)
                     | `Ok res -> (
                         if res.Cohttp.Response.status <> `OK then
                           failwith "test_proxy_connect: HTTP CONNECT failed";
                         let request = "hello" in
                         Outgoing.C.write_string oc request 0
                           (String.length request);
                         Outgoing.C.flush oc >>= function
                         | Error _ ->
                             failwith
                               "http_proxy_connect: client flush returned error"
                         | Ok () -> (
                             Outgoing.C.read_some ~len:5 oc >>= function
                             | Ok `Eof ->
                                 failwith
                                   "http_proxy_connect: client read_some \
                                    returned Eof"
                             | Error _ ->
                                 failwith
                                   "http_proxy_connect: client read_some \
                             ...
                             | Ok (`Data buf) ->
                                 let txt = Cstruct.to_string buf in
                                 Alcotest.check Alcotest.string "message"
                                   "there" txt;
                                 Lwt.pick
                                   [
...
let test_http_proxy_connect_rejected () =
  Host.Main.run
    (Slirp_stack.with_stack ~pcap:"test_http_proxy_connect_rejected.pcap"
       (fun _ stack ->
         let json =
           Ezjsonm.from_string
             {|
      ...
      "allow_error_msg": "stop connecting to %s"
        ...
        |}
         ...
         | Ok () -> (
             ...
             | Ok flow -> (
                 Log.info (fun f ->
                     f "Connected to %s:3128" (Ipaddr.to_string primary_dns_ip));
                 let oc = Outgoing.C.create flow in
                 let request =
                   let connect =
                     Cohttp.Request.make ~meth:`CONNECT (Uri.make ())
                 ...
                 | `Invalid x ->
                     failwith
                       ("test_proxy_connect_rejected: Invalid HTTP response: "
                      ^ x)
                 | `Ok res ->
                     if res.Cohttp.Response.status = `OK then
                       failwith
                         "test_proxy_connect_rejected: HTTP CONNECT accepted";
                     if res.Cohttp.Response.status <> `Forbidden then
                       failwith
                         "test_proxy_connect_rejected: HTTP CONNECT did not \
                          fail with forbidden";
                     Lwt.return (`Result ()))))
     >|= function
     | `Timeout -> failwith "HTTP interception failed"
         ...
         with_server
           ...
           (fun origin_server ->
             ...
             | Ok () -> (
                 ...
                 | Ok flow ->
                     Log.info (fun f ->
                         f "Connected to %a:3128" Ipaddr.pp primary_dns_ip);
                     let oc = Outgoing.C.create flow in
                     let request =
                       let uri =
           ...
           (fun server ->
             ...
             | Ok () -> (
                 (* Create a regular HTTP connection, this should be caught by the transparent
                    proxy *)
                 Client.TCP.create_connection (Client.tcp stack.t)
                   (Ipaddr.of_string_exn host, port)
                 >>= function
                 | Error _ ->
    ...
    ( "HTTP proxy: GET to localhost works",
      [
        ( "check that HTTP GET to localhost via IP",
          `Quick,
          test_http_proxy_localhost (Ipaddr.to_string Slirp_stack.localhost_ip)
        );
      ] );
