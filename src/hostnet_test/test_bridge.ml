...
let get_ips c = Client.Ipv.configured_ips c |> List.map Ipaddr.Prefix.address
  ...
  Host.Main.run
    (let rec loop x used_ips used_macs =
       ...
       | x, used_ips, used_macs ->
               ...
               List.iter
                 (fun ip ->
                   if List.mem ip used_ips then (
                     Log.err (fun f ->
                         f "IP %s is a duplicate" (Ipaddr.to_string ip));
                     assert false))
                 ips;
           ...
           >>= fun (ips, mac) ->
           Log.info (fun f ->
               f "Stack %d got IPs %s and MAC %s" x
                 (String.concat ", " (List.map Ipaddr.to_string ips))
...
let test_reconnect () =
  Host.Main.run
    (let uuid = uuid_gen () in
     ...
     >>= fun (ip, mac) ->
     Log.info (fun f ->
         f "First connection got IP %s and MAC %s" (Ipaddr.to_string ip)
           (Macaddr.to_string mac));
     ...
     Log.info (fun f ->
         f "Reconnect got IP %s and MAC %s"
           (Ipaddr.to_string reconnect_ip)
           (Macaddr.to_string reconnect_mac));
     assert (Ipaddr.compare ip reconnect_ip == 0);
...
let test_connect_preferred_ipv4 preferred_ip () =
  Host.Main.run
    (let uuid = uuid_gen () in
     ...
     assert (Ipaddr.compare ip (V4 preferred_ip) == 0);
     Log.info (fun f ->
         f "First connection got IP %s and MAC %s" (Ipaddr.to_string ip)
           (Macaddr.to_string mac));
