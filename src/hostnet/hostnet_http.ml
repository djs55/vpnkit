...
module Make
  ...
  let to_string t =
    Printf.sprintf "http = %s; https = %s; exclude = %s"
      (match t.http with None -> "None" | Some (ip, port) -> Ipaddr.to_string ip ^ ":" ^ (string_of_int port))
      (match t.https with None -> "None" | Some (ip, port) -> Ipaddr.to_string ip ^ ":" ^ (string_of_int port))
      (String.concat "; " t.exclude)

  let none = { http = None; https = None; exclude = [] }
