...
struct
  ...
  module Tcp = struct
    ...
    module Flow = struct
      ...
      let idle_time t : Duration.t =
        Int64.sub (Mirage_mtime.elapsed_ns ()) t.last_active_time_ns
  ...
  module Endpoint = struct
    ...
    let touch t = t.last_active_time_ns <- Mirage_mtime.elapsed_ns ()
    ...
    let send_icmp_dst_unreachable t ~src ~dst ~src_port ~dst_port ~ihl raw =
      let would_fragment ~ip_header ~ip_payload =
        ...
        Cstruct.BE.set_uint16 header 5 0x00;
        (* Set ID *)
        Cstruct.BE.set_uint16 header 7 safe_outgoing_mtu;
        (* Set seq *)
        let icmp_payload =
  ...
  module Gateway = struct
    ...
    let input_ipv4 t ipv4 =
      ...
      | Ipv4
        ...
        when Gateway_forwards.Tcp.mem dst_port ->
          let id =
            Stack_tcp_wire.v ~src_port:dst_port ~dst:src ~src:dst
              ~dst_port:src_port
          in
          ...
          Endpoint.input_tcp t.endpoint ~id ~syn ~rst
            (Ipaddr.V4 forward_ip, forward_port)
            raw
          >|= ok
      ...
      | Ipv4
          {
            ...
            dst;
            payload =
              Udp { src = src_port; dst = 53; payload = Payload payload; _ };
            _;
          } ->
      ...
      | Ipv4
          ...
          in
          Endpoint.intercept_tcp t.endpoint ~id ~syn ~rst
            (fun () ->
              !dns >>= fun t ->
      ...
      | Ipv4
          ...
          | Some http -> (
              match
                Http_forwarder.explicit_proxy_handler
                  ~localhost_names:t.localhost_names
                  ~localhost_ips:t.localhost_ips ~dst:(dst, dst_port) ~t:http
              with
              | None -> Lwt.return (Ok ())
              | Some cb ->
                  cb >>= fun cb ->
                  Endpoint.intercept_tcp t.endpoint ~id ~syn ~rst
                    (fun _ -> Lwt.return @@ with_no_keepalive cb)
                    raw
                  >|= ok))
      | _ -> Lwt.return (Ok ())
    ...
    let create endpoint udp_nat dns_ips localhost_names localhost_ips =
      let tcp_stack =
        { endpoint; udp_nat; dns_ips; localhost_names; localhost_ips }
      in
      let open Lwt.Infix in
      (* Wire up the listeners to receive future packets: *)
      Switch.Port.listen ~header_size:Ethernet.Packet.sizeof_ethernet
  ...
  module Remote = struct
    ...
    let input_ipv4 t ipv4 =
      ...
      | Ipv4
          {
            ...
            dst;
            ttl;
            payload = Icmp { ty; code; icmp = Echo { id; seq; payload }; _ };
            _;
          } -> (
          let datagram =
            { Hostnet_icmp.src; dst; ty; code; seq; id; payload }
          in
          match t.icmp_nat with
          | Some icmp_nat -> Icmp_nat.input ~t:icmp_nat ~datagram ~ttl () >|= ok
          | None -> Lwt.return (Ok ()))
      ...
      | Ipv4
          {
            ...
            dst = local_ip;
            payload =
              Tcp { src = dest_port; dst = local_port; syn; rst; raw; _ };
            _;
          } -> (
          let id =
            Stack_tcp_wire.v ~src_port:local_port ~dst:dest_ip ~src:local_ip
              ~dst_port:dest_port
          in
          let callback =
            match !http with
            | None -> None
            | Some http ->
                Http_forwarder.transparent_proxy_handler
                  ~localhost_names:t.localhost_names
                  ~localhost_ips:t.localhost_ips ~dst:(local_ip, local_port)
                  ~t:http
          in
          match callback with
          | None ->
      ...
      | Ipv4
          ...
          let description =
            Printf.sprintf "%s:%d -> %s:%d" (Ipaddr.V4.to_string src) src_port
    ...
    let create endpoint udp_nat icmp_nat localhost_names localhost_ips =
      let tcp_stack =
        { endpoint; udp_nat; icmp_nat; localhost_names; localhost_ips }
      in
      let open Lwt.Infix in
      (* Wire up the listeners to receive future packets: *)
      Switch.Port.listen ~header_size:Ethernet.Packet.sizeof_ethernet
        endpoint.Endpoint.netif (fun buf ->
          ...
          | Ok (Ethernet { payload = Ipv4 ipv4; _ }) -> (
              ...
              | Error e ->
                  Log.err (fun l ->
                      l "error while reading IPv4 input: %a" Stack_ipv4.pp_error
                        e))
          | _ -> Lwt.return_unit)
      >|= function
      | Ok () -> Ok tcp_stack
      ...
      let xs =
        IPMap.fold
          (fun (remote, local) t acc ->
            Fmt.str "%a <-> %a last_active_time = %s" Ipaddr.V4.pp remote
              Ipaddr.V4.pp local
              (Duration.pp Format.str_formatter (Endpoint.idle_time t);
               Format.flush_str_formatter ())
            :: acc)
          t.endpoints []
      in
  ...
  let http_intercept_api_handler flow =
    ...
    let write_response code status body =
      ...
      | Error _ ->
          Log.warn (fun f ->
              f "HTTP intercept API: error writing response to client");
          Lwt.return_unit
    in
    Lwt.catch
      (fun () ->
        ...
        | `Ok req -> (
            match
              (Cohttp.Request.meth req, Uri.path @@ Cohttp.Request.uri req)
            with
            | `GET, "/http_proxy.json" -> (
                match !http with
                | None -> write_response "404" "None set" ""
                | Some http ->
                    write_response "200" "OK" (Http_forwarder.to_string http))
            | `POST, "/http_proxy.json" -> (
                ...
                | body -> (
                    ...
                    | exception e ->
                        write_response "400" "Bad_request"
                          ("Failed to parse .json: " ^ Printexc.to_string e)
                    | json -> (
                        Http_forwarder.of_json json >>= function
                        | Error (`Msg m) ->
                            write_response "400" "Bad_request"
          ...
          | `File file -> (
              ...
              let copy () =
                ...
                let rec aux offset =
                  ...
                  Vfs.File.read fd ~offset ~count >>?= fun buf ->
                  fragments := buf :: !fragments;
                  let len = Int64.of_int @@ Cstruct.length buf in
                  if len = 0L then Lwt.return_unit
                  else aux (Int64.add offset len)
                in
                aux 0L >>= fun () -> Lwt.return (List.rev !fragments)
              in
              copy () >>= fun fragments ->
              let length =
  ...
  module Debug = struct
    ...
    let update_dns ?(local_ip = Ipaddr.V4 Ipaddr.V4.localhost)
        ?(builtin_names = []) () =
      let local_address =
    ...
    let update_http ?http:http_config ?https ?exclude ?transparent_http_ports
        ?transparent_https_ports () =
      Http_forwarder.create ?http:http_config ?https ?exclude
        ?transparent_http_ports ?transparent_https_ports ()
      >>= function
  ...
  let rec delete_unused_endpoints t ~port_max_idle_time () =
    if port_max_idle_time <= 0 then Lwt.return_unit (* never delete a port *)
    else
      Mirage_sleep.ns (Duration.of_sec 30) >>= fun () ->
      let max_age = Duration.of_sec port_max_idle_time in
      Lwt_mutex.with_lock t.endpoints_m (fun () ->
          let old_ips =
            IPMap.fold
              (fun (remote, local) endpoint acc ->
                let idle_time = Endpoint.idle_time endpoint in
                if idle_time > max_age then (
                  Log.info (fun f ->
                      f "expiring endpoint %a <-> %a with idle time %s > %s"
                        Ipaddr.V4.pp remote Ipaddr.V4.pp local
                        (Duration.pp Format.str_formatter idle_time;
                         Format.flush_str_formatter ())
                        (Duration.pp Format.str_formatter max_age;
                         Format.flush_str_formatter ()));
          ...
          in
          Lwt_list.iter_s
            (fun ((remote, local), endpoint) ->
              Switch.remove t.switch remote;
              t.endpoints <- IPMap.remove (remote, local) t.endpoints;
              Endpoint.destroy endpoint)
            old_ips)
      >>= fun () -> delete_unused_endpoints t ~port_max_idle_time ()

  let connect x vnet_switch vnet_client_id client_macaddr c
      (global_arp_table : arp_table) =
    ...
    Netif.connect filteredif |> fun interface ->
    Dns_forwarder.set_recorder interface;

    let kib = 1024 in
    (* Capture 256 KiB of all traffic *)
    let all_traffic =
      Netif.add_match ~t:interface ~name:"all.pcap" ~limit:(256 * kib)
        ~snaplen:c.Configuration.pcap_snaplen ~predicate:(fun _ -> true)
    in
    ...
    in
    Global_arp_ethif.connect switch >>= fun global_arp_ethif ->
    ...
    Lwt.catch
      ...
      (function
        | Unix.Unix_error (Unix.EPERM, _, _) ->
            Log.err (fun f ->
                f
                  "Permission denied setting up user-space ICMP socket: ping \
                   will not work");
            Lwt.return None
        | e ->
            Log.err (fun f ->
                f
                  "Unexpected exception %s setting up user-space ICMP socket: \
                   ping will not work"
    ...
    in
    Lwt.async
    @@ delete_unused_endpoints
         ~port_max_idle_time:c.Configuration.port_max_idle_time t;

    let find_endpoint (remote, local) =
      Lwt_mutex.with_lock t.endpoints_m (fun () ->
          if IPMap.mem (remote, local) t.endpoints then
            Lwt.return (Ok (IPMap.find (remote, local) t.endpoints))
          else
            Endpoint.create interface switch local_arp_table remote local
              c.Configuration.mtu
            >|= fun endpoint ->
    ...
    let send_reply = function
      | {
          ...
          let src =
            if Ipaddr.V4.compare src Ipaddr.V4.localhost = 0 then
              c.Configuration.host_ip
            else src
          in
          ...
          | Error (`Msg m) ->
              ...
              Lwt.return_unit
          | Ok endpoint -> (
              Stack_udp.write ~src_port ~dst ~dst_port endpoint.Endpoint.udp4
                payload
              >|= function
              | Error e ->
                  Log.err (fun f ->
      ...
      | { Hostnet_udp.src = src, src_port; dst = dst, dst_port; _ } ->
          Log.err (fun f ->
              f "Failed to send non-IPv4 UDP datagram %a:%d -> %a:%d" Ipaddr.pp
                src src_port Ipaddr.pp dst dst_port);
          Lwt.return_unit
    in
    ...
    Vnet.set_listen_fn vnet_switch t.vnet_client_id (fun buf ->
        ...
        | Ok (Ethernet { src = eth_src; dst = eth_dst; _ }) -> (
            ...
            Switch.write ~size:Ethernet.Packet.sizeof_ethernet switch
              (fun toBuf ->
    ...
    Log.info (fun f ->
        f "Client mac: %s server mac: %s"
          (Macaddr.to_string client_macaddr)
          (Macaddr.to_string c.Configuration.server_macaddr));
    Switch.listen switch ~header_size:Ethernet.Packet.sizeof_ethernet
      (fun buf ->
        ...
        | Ok (Ethernet { src = eth_src; dst = eth_dst; _ })
          when not
                 (Macaddr.compare eth_dst client_macaddr = 0
            ...
            Vnet.write vnet_switch t.vnet_client_id ~size:c.Configuration.mtu
              (fun toBuf ->
                Cstruct.blit buf 0 toBuf 0 (Cstruct.length buf);
                Cstruct.length buf)
            >|= function
            ...
            | Error e ->
                Log.err (fun l ->
                    l "Vnet write failed: %a" Mirage_net.Net.pp_error e))
        | Ok
            (Ethernet
              {
                dst = eth_dst;
