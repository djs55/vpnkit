...
let forward_of_json j =
  ...
  let dst_prefix =
    match
      Ipaddr.Prefix.of_string @@ get_string @@ find j [ "dst_prefix" ]
    with
    | Error (`Msg m) ->
  ...
  module Request = struct
    ...
    let of_json j =
      ...
      let dst_ip =
        ...
        | Error (`Msg m) ->
            raise (Parse_error (j, "dst_ip should be an IPv4 address: " ^ m))
        | Ok x -> x
      in
      let dst_port = get_int @@ find j [ "dst_port" ] in
    ...
    Host.Sockets.Stream.Unix.listen s (fun flow ->
        ...
        | Ok h -> (
            ...
            Host.Sockets.Stream.Tcp.connect
              ( h.Handshake.Request.dst_ip,
                h.Handshake.Request.dst_port )
            >>= function
            | Error (`Msg m) -> (
