...
struct
  ...
  module Dns_tcp_resolver = Dns_forward.Resolver.Make (Dns_tcp_client)
  ...
  module Dns_udp_resolver = Dns_forward.Resolver.Make (Dns_udp_client)
  ...
  let record_udp ~source_ip ~source_port ~dest_ip ~dest_port bufs =
    ...
    | Some recorder ->
        ...
        Ipv4_wire.set_id buf (Random.int 65535);
        (* TODO *)
        ...
        | `Dont_know -> (
            ...
            | None -> (
                ...
                | _, Host -> (
                    ...
                    | Ok answers -> Lwt.return (Ok (marshal_reply answers))
                    (* Currently return all errors as NXDOMAIN *)
                    | Error _ -> Lwt.return (Ok (Some (marshal nxdomain)))))))
