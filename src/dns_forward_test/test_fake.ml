...
let test_good_dead_server () =
  ...
  match
    Lwt_main.run
      ...
      Mirage_mtime_set.tick_for Duration.(of_ms 500);
...
let tests = [ ("Tolerate broken server", `Quick, test_good_dead_server) ]
...
let () =
  ...
  Alcotest.run "dns-forward-fake" [ ("Test infrastructure", tests) ]
